...
const WINDOW_MANAGER_KEY = Symbol('window_manager');

export function set_window_manager_context<T extends string>() {
	const manager = new WindowManager<T>();
	return setContext(WINDOW_MANAGER_KEY, manager);
}

export function get_window_manager_context<T extends string>() {
	return getContext<WindowManager<T>>(WINDOW_MANAGER_KEY);
}
